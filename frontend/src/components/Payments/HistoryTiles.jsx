...
const HistoryTiles = ({ payment , searchItem}) => {
  ...
  const date = `${d.getUTCDate().toString().padStart(2, "0")}-${(
    ...
    .toString()
    .padStart(2, "0")}-${d.getUTCFullYear()} ${h
    .toString()
    .padStart(2, "0")}:${m} ${ampm}`;
  ...
  return (
    <>
      {receiver && receiver.firstname.toLowerCase().startsWith(searchItem.toLowerCase()) && (
        <div className="mr-20 flex flex-row gap-5 max-sm-gap-0 max-md:gap-1  max-sm:mr-0 max-md:mr-10">
          <img
            className="mt-1 p-1 w-10 h-10 rounded-full ring-2 ring-green-700"
            src={
              receiver?.avatar?.url ||
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-row border border-success bg-green-500 w-fit px-2 rounded-xl items-center">
              ...
              <p className="max-sm:hidden text-xs text-black text-center font-bold">
                Transfers
              </p>
            </div>
